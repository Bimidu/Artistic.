...
async def save_report(
    ...
    Requires authentication. Reports are associated with the current user.
    """
    db = get_database()
...
async def get_my_reports(
    ...
    Returns a list of all patient analysis reports created by the authenticated user,
    sorted by creation date (newest first).
    """
    db = get_database()

    # Find all reports for this user
    cursor = db.patient_reports.find({"user_id": current_user.id}).sort("created_at", -1)
...
async def get_reports_by_patient(
    ...
    Returns reports organized by patient, with each patient having a list
    of all their reports sorted by date (newest first).
    """
    db = get_database()
